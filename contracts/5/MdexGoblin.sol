...
contract MdexGoblin is Ownable, ReentrancyGuard, IGoblin {
    ...
    function work(
        ...
        require(
            borrowToken == token0 || borrowToken == token1 || borrowToken == address(0),
        ...
        IStrategy(strategy).execute.value(msg.value)(user, borrowToken, borrow, debt, ext);
    ...
    function health(uint256 id, address borrowToken) external view returns (uint256) {
        ...
        if (borrowToken == token0) {
            return
                getMktSellAmount(userToken1, totalAmount1.sub(userToken1), totalAmount0.sub(userToken0)).add(
                    userToken0
                );
        } else {
            return
                getMktSellAmount(userToken0, totalAmount0.sub(userToken0), totalAmount1.sub(userToken1)).add(
                    userToken1
                );
        }
    }
